...
public:
    ...
    static inline void delayMicros(unsigned long delay_us, unsigned long start_us = 0){
        if (delay_us){
            ...
            while ((delay_us - min(micros() - start_us, delay_us)) > MIN_YIELD_MICROS){
                // Serial.print("Yield Delay ");
                // Serial.print(delay_us);
                // Serial.print(" ");
                // Serial.println((delay_us - min(micros() - start_us, delay_us)));
                vTaskDelay(0);
            }
            // See https://www.gammon.com.au/millis
            while (micros() - start_us < delay_us); //Elapsed < GoalDelay
        }
    }
...
public:
     ...
     */
    void begin(float rpm=660, short microsteps=1);
    /*
